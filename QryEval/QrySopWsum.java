    ...
    public double getDefaultScore (RetrievalModel r, long docid) throws IOException {
        if (r instanceof RetrievalModelIndri) {
            ...
            for (int i = 0; i < this.args.size(); i++) {
                ...
                result += weights.get(i)/totalW * q_i.getDefaultScore(r, docid);
            }
            return result;
        } else {
            throw new IllegalArgumentException
                    (r.getClass().getName() + " doesn't support the WSUM operator default score.");
        }
    }
        ...
        if (! this.docIteratorHasMatchCache()) {
            ...
            for (int i = 0; i < this.args.size(); i++) {
                ...
                if (q_i.docIteratorHasMatch (r) && q_i.docIteratorGetMatch () == docid) {
                    result += weights.get(i)/totalW * q_i.getScore(r);
                } else {
                    result += weights.get(i)/totalW * q_i.getDefaultScore(r, docid);
                }
            }
